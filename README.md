...
- **Cross-platform** -- builds and tests on macOS, Linux, and Windows (CI on all three). The same core runs inside the macOS CoreAudio HAL driver process (coreaudiod, alongside libASPL), directly inside the app process on Linux (with PortAudio), or linked into the Unreal Engine plugin on any platform. Audio never crosses process boundaries -- all ring buffers are in-process SPSC.
